...
$if %task% == "load_params" $if %switch_vertical_integration% == 1 $goto task_params_vi
$if %task% == "load_params" $if not %switch_vertical_integration% == 1 $goto task_params
...
- ExcelReader:
    ...
    symbols:
      ...
      - name: TagExogenousRegion
        range: "Par_TagExogenousRegion!A2"
        rowDimension: 1
        columnDimension: 0
